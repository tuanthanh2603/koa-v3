...
<template>
  ...
  <header v-if="isHydrated" :class="[
    ...
    <div class="border-b transition-all duration-500 ease-in-out" :class="[
      isScrolled
        ? colorMode.value === 'dark'
          ? 'border-slate-700/50'
          : 'border-white/10'
        ...
        <div :class="[
          ...
          <div class="flex flex-1">
            ...
            <button @click="toggleDarkMode" :class="[
              // thêm hidden md:flex để ẩn trên mobile và hiển thị từ md trở lên
              'hidden md:flex group items-center gap-2 px-4 py-2 rounded-full transition-all duration-300 hover:scale-105',
              isScrolled
                ? colorMode.value === 'dark'
                  ? 'text-slate-200 hover:bg-slate-800'
          ...
          <div class="flex-1 flex justify-center">
            <NuxtLink to="/" class="shrink-0">
              <img :src="colorMode.value === 'dark' ? ImageLogoDark : ImageLogo" alt="Logo"
                class="transition-all duration-500 ease-in-out mt-1" :class="[
                  isScrolled
          ...
          <div class="flex-1 flex justify-end items-center gap-2">
            ...
            <div :class="[
              ...
              <UButton icon="i-heroicons-magnifying-glass" variant="ghost"
                :color="colorMode.value === 'dark' ? 'gray' : 'neutral'" square @click="isSearchOpen = true" />
            ...
            <UButton icon="i-heroicons-magnifying-glass" variant="ghost"
              :color="colorMode.value === 'dark' ? 'gray' : 'neutral'" class="md:hidden" @click="isSearchOpen = true"
              square />
            <UButton :icon="isMobileMenuOpen ? 'i-heroicons-x-mark' : 'i-heroicons-bars-3'" variant="ghost"
              :color="colorMode.value === 'dark' ? 'gray' : 'neutral'" class="md:hidden"
              @click="isMobileMenuOpen = !isMobileMenuOpen" square />
          </div>
        </div>

        <!-- Desktop Navigation Menu -->
        <nav class="hidden md:flex items-center justify-center h-16 space-x-10 transition-all duration-500"
        ...
        <div v-if="isMobileMenuOpen" class="md:hidden px-2 pb-3">
          <div :class="[
            ...
            <nav class="flex flex-col">
              <NuxtLink v-for="item in menuItems" :key="`mobile-${item.labelKey}`" :to="item.to" :class="[
    ...
    <div v-if="isSearchOpen" class="fixed inset-0 z-60" aria-modal="true" role="dialog"
      ...
      <div :class="[
        ...
        <div class="flex items-center justify-between mb-4">
          ...
          ]" aria-label="Close search" @click="isSearchOpen = false">
            <UIcon name="i-heroicons-x-mark" :class="[
              'w-5 h-5',
        ...
        <div class="relative w-full mb-4">
          ...
          <input v-model="searchQuery" :placeholder="selectedLang === 'vi' ? 'Nhập tên dự án' : 'Enter project name'"
            autofocus @input="performSearch" @keyup.enter="performSearch" :class="[

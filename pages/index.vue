...
const showLogo2 = ref(false) // Thêm state này
const menuItems = [
...
onMounted(() => {
  // Sau 1.5 giây bắt đầu fade out
  setTimeout(() => {
    fadeOut.value = true
  }, 1500)

  // Sau 2 giây ẩn hoàn toàn splash screen
  setTimeout(() => {
    showSplash.value = false
    showLogo2.value = true // Hiển thị logo2 với animation
  ...
  <div class="h-screen overflow-y-scroll snap-y snap-mandatory scroll-smooth">
    <section class="relative h-screen snap-start flex items-center justify-center  text-white text-3xl">
      ...
      <Transition enter-active-class="transition-all duration-700 ease-out" enter-from-class="opacity-0 scale-75"
        ...
        <div v-if="showLogo2" class="absolute top-[5%] left-1/2 transform -translate-x-1/2 z-50">
      ...
      <Transition enter-active-class="transition-all duration-700 ease-out" enter-from-class="opacity-0 scale-75"
        enter-to-class="opacity-100 scale-100">
        <button v-if="showLogo2"
          class="text-sm absolute bottom-[24%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-40 px-8 py-1 border-2 border-white text-white font-semibold hover:bg-white hover:text-black transition duration-300 flex items-center gap-2 group">
          Khám phá
